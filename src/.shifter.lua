...
shifter.shift_rows =
    function(lower_row_amount, upper_row_amount, lower_shift_amount, upper_shift_amount)
        local shifted_image
        ...
        if active_cel.image.width < app.activeSprite.width then
            ...
            shifted_image = shifter.expand_cel_bounds(
                active_cel,
                app.activeSprite.width,
                active_cel.image.height,
                active_cel.position.x,
                0,
                0,
                active_cel.position.y
            ).image
        else
        ...
        local rows_to_shift = shifter.get_row_indices(
            row_amount,
            shifted_image.height
        )
        ...
        for i = 1, row_amount, 1 do
            ...
            local shift_amount = math.random(
                lower_shift_amount,
                upper_shift_amount
            )
...
shifter.show = function(x, y)
    ...
    dialog
        ...
        :radio({
            ...
            text = "Range",
            onclick = function()
                util.toggle_ui_elements(
                    dialog.data.row_fixed,
                    { "fixed_row_amount" },
                    dialog
                )
                util.toggle_ui_elements(
                    dialog.data.row_range,
                    { "upper_row_amount", "lower_row_amount" },
                    dialog
                )
            end,
        })
        ...
        :radio({
            ...
            onclick = function()
                util.toggle_ui_elements(
                    dialog.data.shift_fixed,
                    { "fixed_shift_amount" },
                    dialog
                )
                util.toggle_ui_elements(
                    dialog.data.shift_range,
                    { "upper_shift_amount", "lower_shift_amount" },
                    dialog
                )
            end,
        })
